...
// getCurrentImage returns the current image by merging all adds and deletes.
func (l *LayeredMap) getCurrentImage() map[string]string {
	if l.isCurrentImageValid {
		return l.currentImage
	}

	if l.currentImage == nil {
		l.currentImage = make(map[string]string)
	}

	// Apply any unmerged layers on top of currentImage
	if len(l.adds) > 0 {
		lastIdx := len(l.adds) - 1
		for add, hash := range l.adds[lastIdx] {
			l.currentImage[add] = hash
		}
		if lastIdx < len(l.deletes) {
			for del := range l.deletes[lastIdx] {
...
// updateCurrentImage updates the internal current image by merging the
...
func (l *LayeredMap) GetCurrentPaths() map[string]struct{} {
	...
	paths := make(map[string]struct{}, len(current))
	for f := range current {
		paths[f] = struct{}{}
	}
	return paths
