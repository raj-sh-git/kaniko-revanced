...
[![Security Scan](https://github.com/raj-sh-git/kaniko-revanced/actions/workflows/security-scan.yaml/badge.svg)](https://github.com/raj-sh-git/kaniko-revanced/actions/workflows/security-scan.yaml)
[![Go Report Card](https://goreportcard.com/badge/github.com/raj-sh-git/kaniko-revanced)](https://goreportcard.com/report/github.com/raj-sh-git/kaniko-revanced)
[![Docker Hub](https://img.shields.io/docker/pulls/kanikorevanced/executor.svg)](https://hub.docker.com/r/kanikorevanced/executor)

![kaniko logo](logo/Kaniko-Logo.png)

> **Notice**: This is a fork of original Kaniko. As the original project (`GoogleContainerTools/kaniko`) has been archived, I am starting this project as **kaniko-revanced** where I will fix CVEs, improve performance, and continue the Kaniko legacy.

## Key Features

- **Active CVE Remediation & Security Scanning**: Automated vulnerability audits with Trivy, Grype, and `govulncheck`, combined with prompt security patching.
- **Performance Optimizations**: Incremental layer snapshotting and `sync.Pool` buffer pooling for accelerated layer compression and change detection.
- **Regular Bi-Weekly Releases**: Automated release cadence with changelogs and SLSA/Cosign provenance.
- **100% Drop-In Compatible**: Works directly in existing Kubernetes Pods, GitLab CI, Tekton Pipelines, Argo Workflows, and GitHub Actions without modifying build flags.

## Performance Benchmarks

In comparative testing against the official Google Kaniko executor (`gcr.io/kaniko-project/executor:debug`), `kaniko-revanced` delivers significantly faster and more consistent build times by eliminating duplicate memory allocations during layer snapshotting and reusing buffer pools during archive serialization.

### Benchmark Setup & Workload
- **Multi-Stage Build**: 3 distinct stages (`builder` -> `packager` -> `runtime`).
- **Filesystem Node Stress**: Dynamic creation and tracking of 2,500 distinct files across nested Go package trees.
- **Binary I/O & Buffering**: Generation and checksum verification of a 20MB payload to measure tar streaming throughput.
- **Cross-Stage Extraction**: `COPY --from=...` and artifact packaging.

### Results Comparison

| Metric | Original Kaniko (`gcr.io/...`) | kaniko-revanced (`kanikorevanced/executor:...`) | Outcome |
| :--- | :--- | :--- | :--- |
| **Best Execution Time** | `8.08s` | **`7.86s`** | **Faster** |
| **Average Execution Time** | `17.34s` | **`8.25s`** | **52.4% faster (2.10x speedup)** |
| **Run 1** | `8.08s` | **`7.86s`** | Stable |
| **Run 2** | `35.46s` | **`8.40s`** | No snapshot degradation |
| **Run 3** | `8.48s` | **`8.51s`** | Stable |
| **Artifact Compatibility** | `3.91MB` | `3.91MB` | **100% Bit-compatible** |
...
spec:
  ...
  - name: kaniko
    ...
    - "--context=git://github.com/your-org/your-repo.git"
    - "--destination=docker.io/your-org/your-image:tag"
    volumeMounts:
    - name: kaniko-secret
...
1. Add an environment variable `STACKLOG_PATH` to your
   [pod definition](https://github.com/raj-sh-git/kaniko-revanced/blob/master/examples/pod-build-profile.yaml#L15).
...
2. Kaniko needs to be able to run on the desired architectures. At the time of
   ...
   [linux/amd64, linux/arm64, linux/s390x and linux/ppc64le (not on \*-debug images)](https://github.com/raj-sh-git/kaniko-revanced/blob/main/.github/workflows/images.yaml).
...
occur, please
[open an issue](https://github.com/raj-sh-git/kaniko-revanced/issues)._
